...
void reset(Reg *reg, int *cont, int *ciclo);

void inputJanelaArquivo(char *buffer, int maxlen);
int carregaMemInst(char mi[256][17], int *cont, int *ciclo, Reg *reg);
void carregarMemoriaDados(int md[256], int *cont, int *ciclo, Reg *reg);
...
int NOP(int opcode, int op_ant, int rt, int rd, int escreg, int escmem, int flag, int *reg, int *mem, Decod *decod, int *cont);
...
int main() {
	...
	do {
		...
		switch (op) {
		case 1:
			carregaMemInst(mi,&cont,&ciclo,&reg);
			break;
		case 2:
			carregarMemoriaDados(md,&cont,&ciclo,&reg);
			break;
		case 3:
...
int executa_ciclo(char mi[256][17],Inst *inst,Decod *decod,Reg *reg,int *md,Sinais *sinais,ULA_Out *ula_out,int *ciclo,Stack *stack,int *cont, UF *uf) {
	...
	NOP(decod->opcode, reg->id_ex.opcode, decod->rt, decod->rd, sinais->EscReg, sinais->EscMem,ula_out->flag_zero,&reg->id_ex.escreg,&reg->id_ex.escmem, decod, cont);
