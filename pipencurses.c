...
void visualizarPipeline(Reg *reg, int ciclo, UF *uf);
...
int executa_ciclo(char mi[256][17],Inst *inst,Decod *decod,Reg *reg,int *md,Sinais *sinais,ULA_Out *ula_out,int *ciclo,Stack *stack,int *cont, UF *uf) {
  ...
  visualizarPipeline(reg, *ciclo - 1, uf);
}
...
void visualizarPipeline(Reg *reg, int ciclo, UF *uf){
  ...
  printf("\n=== VISUALIZAÇÃO DO PIPELINE (Ciclo %d) ===\n", ciclo);
  printf("+-----------+---------------------+-------------------------------------+\n");
  printf("| Estágio   | Instrução Atual     | Detalhes da Execução                |\n");
  printf("+-----------+---------------------+-------------------------------------+\n");

  //estagio if - busca da instrucao
  printf("| Busca     | ");
  if (strcmp(reg->if_id.inst, "0000000000000000") != 0){
    printf("%-19s | ", reg->if_id.inst);

    //decodifica para mostrar a instrucao em assembly
    decodificarInstrucao(reg->if_id.inst, &inst_temp, &decod_temp);
    printf("Busca: ");
    printInstrucao(&decod_temp);
  }
    
  else{
    printf("NOP                 | Estágio vazio ou instrução NOP");
  }
  printf("\n");

  //estagio id - decodificacao
  printf("| Decodifica| ");
  if (reg->id_ex.opcode != 0 || reg->id_ex.rd != 0){
      
    switch(reg->id_ex.opcode) {
      case 0:  printf("Tipo R             | ");
      break;
      case 4:  printf("ADDI               | ");
      break;
      case 8:  printf("BEQ                | ");
  ...
  if (reg->ex_mem.rd != 0 || reg->ex_mem.escmem) {
    if (reg->ex_mem.escmem){
      printf("SW                  | ");
      printf("Calculando endereço: %d + %d", reg->id_ex.a, reg->id_ex.imm);
    }
      ...
      switch(reg->id_ex.opula) {
        case 0:
          printf("ADD: %d + %d", reg->id_ex.a, ULAFonteB(reg->id_ex.b, reg->ex_mem.saidaula, MemReg(reg->mem_wb.saidaula, reg->mem_wb.dadomem, reg->mem_wb.memreg), reg->id_ex.imm, reg->id_ex.ulafonte));
          break;
        
        case 2:
          printf("SUB: %d - %d", reg->id_ex.a, ULAFonteB(reg->id_ex.b, reg->ex_mem.saidaula, MemReg(reg->mem_wb.saidaula, reg->mem_wb.dadomem, reg->mem_wb.memreg), reg->id_ex.imm, reg->id_ex.ulafonte));
          break;
                
        case 4: 
          printf("AND: %d & %d", reg->id_ex.a, ULAFonteB(reg->id_ex.b, reg->ex_mem.saidaula, MemReg(reg->mem_wb.saidaula, reg->mem_wb.dadomem, reg->mem_wb.memreg), reg->id_ex.imm, reg->id_ex.ulafonte));
          break;
                
        case 5: 
          printf("OR: %d | %d", reg->id_ex.a, ULAFonteB(reg->id_ex.b, reg->ex_mem.saidaula, MemReg(reg->mem_wb.saidaula, reg->mem_wb.dadomem, reg->mem_wb.memreg), reg->id_ex.imm, reg->id_ex.ulafonte));
          break;
                
        default: 
          printf("Operação Desconhecida"); break;
      }
    }
  } 
  ...
  if (reg->ex_mem.escreg && reg->ex_mem.memreg == 0) {
    printf("LW                  | ");
    printf("Lendo mem[%d]", reg->ex_mem.saidaula);
  }
  ...
  //estagio wb - escrita no banco de registradores
  printf("| Escreve   | ");
    if (reg->mem_wb.escreg){
  ...
  printf("\nLegenda:\n");
  printf("- NOP: Estágio vazio ou instrução que não faz nada\n");
  printf("- Tipo R: Instruções como add, sub, and, or\n");
  printf("- lw/sw: Acesso à memória (load/store)\n");
...
void infoWin(Reg *reg) {
	initscr();              // Inicia ncurses
  curs_set(0);            // Oculta o cursor
	WINDOW *infowin = newwin(47, 34, 0, 50); // altura, largura, pos_y, pos_x
	int line = 1;
	werase(infowin);
    box(infowin, 0, 0);
    
	mvwprintw(infowin, line++, 1, "REGISTRADORES");
    for (int i = 0; i < 8; i++)
	{
		mvwprintw(infowin, line++, 5, "REG[%d] ---- %d", i, reg->br[i]);
	}
	line++;
	mvwprintw(infowin, line++, 1, "IF -> ID");
	mvwprintw(infowin, line++, 3, "PC --------- %d", reg->if_id.pc);
	mvwprintw(infowin, line++, 3, "INST ------- %s", reg->if_id.inst);
	line++;
	mvwprintw(infowin, line++, 1, "ID -> EX");
	mvwprintw(infowin, line++, 3, "memreg ----- %d", reg->id_ex.memreg);
	mvwprintw(infowin, line++, 3, "escreg ----- %d", reg->id_ex.escreg);
	mvwprintw(infowin, line++, 3, "branch ----- %d", reg->id_ex.branch);
	mvwprintw(infowin, line++, 3, "jump ------- %d", reg->id_ex.jump);
	mvwprintw(infowin, line++, 3, "escmem ----- %d", reg->id_ex.escmem);
	mvwprintw(infowin, line++, 3, "regdest ---- %d", reg->id_ex.regdest);
	mvwprintw(infowin, line++, 3, "ulafonte --- %d", reg->id_ex.ulafonte);
	mvwprintw(infowin, line++, 3, "opula ------ %d", reg->id_ex.opula);
	mvwprintw(infowin, line++, 3, "opcode ----- %d", reg->id_ex.opcode);
	mvwprintw(infowin, line++, 3, "a ---------- %d", reg->id_ex.a);
	mvwprintw(infowin, line++, 3, "b ---------- %d", reg->id_ex.b);
	mvwprintw(infowin, line++, 3, "imm -------- %d", reg->id_ex.imm);
	mvwprintw(infowin, line++, 3, "rs --------- %d", reg->id_ex.rs);
	mvwprintw(infowin, line++, 3, "rt --------- %d", reg->id_ex.rt);
	mvwprintw(infowin, line++, 3, "rd --------- %d", reg->id_ex.rd);
	line++;
	mvwprintw(infowin, line++, 1, "EX -> MEM");
	mvwprintw(infowin, line++, 3, "memreg ----- %d", reg->ex_mem.memreg);
	mvwprintw(infowin, line++, 3, "escreg ----- %d", reg->ex_mem.escreg);
	mvwprintw(infowin, line++, 3, "escmem ----- %d", reg->ex_mem.escmem);
	mvwprintw(infowin, line++, 3, "saidaula --- %d", reg->ex_mem.saidaula);
	mvwprintw(infowin, line++, 3, "b ---------- %d", reg->ex_mem.b);
	mvwprintw(infowin, line++, 3, "rd --------- %d", reg->ex_mem.rd);
	line++;
	mvwprintw(infowin, line++, 1, "MEM -> WB");
	mvwprintw(infowin, line++, 3, "memreg ----- %d", reg->mem_wb.memreg);
	mvwprintw(infowin, line++, 3, "escreg ----- %d", reg->mem_wb.escreg);
	mvwprintw(infowin, line++, 3, "dadomem ---- %d", reg->mem_wb.dadomem);
	mvwprintw(infowin, line++, 3, "saidaula --- %d", reg->mem_wb.saidaula);
	mvwprintw(infowin, line++, 3, "rd --------- %d", reg->mem_wb.rd);
