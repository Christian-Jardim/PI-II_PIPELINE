...
void executa_pipeline_ciclo(char mi[256][17], Inst *inst, Decod *decod, Reg *reg, int *md, Sinais *sinais, ULA_Out *ula_out, int *ciclo);
...
int main() {
  ...
  do {
    ...
    case 8:
      break;
    case 9:executa_pipeline_ciclo(mi, &inst, &decod, &reg, md, &sinais, &ula_out, &ciclo);
      break;
    case 10:
...
void executa_ciclo(char mi[256][17], Inst *inst, Decod *decod, Reg *reg, int *md, Stack *stack, Sinais *sinais, ULA_Out *ula_out) {
  ...
  int dado_mem = 0, dadoA = reg->br[decod->rs], dadoB = reg->br[decod->rt], destino, dado_final, entrada B;
  ...
  if (sinais->EscReg == 1) {
    destino = RegDest(decod->rd, decod->rt, sinais->RegDest);
    dado_final = MemReg(ula_out->resultado, dado_mem, sinais->MemParaReg);
  ...
  // executa
  entradaB = ULAFonte(dadoB, decod->imm, sinais->ULAFonte);
...
void executa_pipeline_ciclo(char mi[256][17], Inst *inst, Decod *decod, Reg *reg, int *md, Sinais *sinais, ULA_Out *ula_out, int *ciclo){
  ...
  if (reg->mem_wb.escreg){
    dado = MemReg(reg->ex_mem.saidaula, md[reg->ex_mem.saidaula], reg->mem_wb.memreg);
    reg->br[reg->ex_mem.rd] = dado;
